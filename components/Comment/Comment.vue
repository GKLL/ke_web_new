 ...
 * @Last Modified time: 2018-06-20 19:12:52
 */
<template>
      ...
      <ul>
        <li v-for="(item, index) in comment.data" :key="index">
          ...
          <div class="user-comment-content">
            ...
            <p class="comment-desc">{{item.commentDesc}}</p>
          </div>
        </li>
      </ul>
...
export default {
  ...
  methods: {
    ...
    submitComment () {
      if (!this.isDisabled) {
        this.$emit('submitComment', {commentDesc: this.textarea})
      }
    },
    // 清空评论框
    clearComment(){
    ...
    textarea (newValue, oldValue) {
      ...
      if( newValue.trim() === '' ) {
        ...
        this.errorMessage = ''
         ...
         this.isDisabled = true
         this.errorMessage = "长度不能超出500个字符"
      } else {
