 ...
 * @Last Modified time: 2018-06-21 16:07:08
 */
<template>
  <section class="news-flash-list">
    ...
    <el-tabs v-model="activeTabName">
      ...
      <el-tab-pane :label="item.tabName" :name="item.tabName" v-for="(item,index) in groupLists" :key="index">
        <el-row class="new-news">有8条新快讯，点击查看</el-row>
        <ul>
          <li v-for="(newsItem,newsIndex) in item.list" :key="newsIndex">
            <div class="news-item-content">
              ...
              <p class="news-item-share">
                ...
                <span :ref="'weixin' + index + '-' + newsIndex" @mouseover="showQRCode(index, newsIndex)" @mouseout="hideQRCode()" title="分享到微信">
                  <svg-icon iconClass="weixin" class="icon-style"></svg-icon>
                </span>
              </p>
              <a v-if="newsItem.newsflashLink" :href="newsItem.newsflashLink" target="_blank" class="news-item-detail">查看原文 ></a>
            </div>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    changeOverflow(item) {
      ...
      if(item.isSpread) item.max = Infinity
      else item.max = 130
    },
    newsflashTextFilter(item) {
      ...
      if(text.length > item.max) {
        item.overflow = true // 溢出
        item.isSpread = false // 未展开状态
    ...
    initQRCode(){
      ...
      // location.protocol + '//' + location.host + '/h5Article/' + this.articleId
      // 生成二维码
      new QRCode(code, {
        text: this.sharePath.weixin,
        width: 100,
        height: 100,
        colorDark: "#000000",
        colorLight: "#ffffff"
