...
export default function ({ req, res, store, route, redirect }) {
  if (route.fullPath.indexOf('/personal') > -1 && !req.session.KGUser) {
    redirect('/')
  } else if (route.fullPath.indexOf('/personal') > -1 && req.session.KGUser) {
    axios.defaults.headers['token'] = req.session.KGUser.token
  }

  if (!route.fullPath.indexOf('/personal') > -1 && req && !req.session.KGUser) {
