 ...
 * @Last Modified by: cb
 * @Last Modified time: 2018-06-22 15:26:08
 */
<template>
  <section class="information-details">
    <section v-if="getAdviertisement('41').imageAddress" class="banner-top">
      <a :href="getAdviertisement('41').adverLink">
        <img v-lazy="handleImgAddrHandle(getAdviertisement('41').imageAddress)" :alt="getAdviertisement('41').adverTitle">
      </a>
    ...
    <section class="content" ref="position">
      ...
      <main>
        <section class="article">
            ...
            <section class="time-type-column">
              ...
              <section v-if="articleInfo.articleType === 2" class="source-from">本文来源：{{articlefromFilter}}</section>
              <a v-if="articleInfo.articleType === 2" class="read-origin" :href="articleInfo.articleLink">阅读原文</a>
          ...
          <section class="upArticle">
            <a v-if="articleInfo.upArticleTitle" :href="'./' + articleInfo.upArticleId" target="_blank">上一篇：{{articleInfo.upArticleTitle}}</a>
            <div v-if="!articleInfo.upArticleTitle">上一篇：没有了</div>
          </section>
          <section class="downArticle">
            <a v-if="articleInfo.downArticleTitle" :href="'./' + articleInfo.downArticleId" target="_blank">下一篇：{{articleInfo.downArticleTitle}}</a>
            <div v-if="!articleInfo.upArticleTitle">上一篇：没有了</div>
          </section>
      ...
      <aside>
        <section class="author-info">
          ...
          <div v-if="authorInfo.identityTag" class="tag-name">{{authorInfo.identityTag}}</div>
          <div class="signature">{{authorInfo.columnIntro}}</div>
          <div class="count">
            <div class="item">
        ...
        <section v-if="getAdviertisement('42').imageAddress" class="banner-right">
          ...
          <div v-if="getAdviertisement('42').imageType == 1" class="banner-title"><a :href="getLinkUrl('42')">{{getAdviertisement('42').articleTitle}}</a></div>
        </section>
...
import { handleImgAddr , formatMsgTime } from '@/utils/utils'
...
export default {
  ...
  data () {
    return {
      ...
      declear: '免责声明：本篇内容由入驻千氪的作者发布，不代表千氪的观点和立场。', // 声明
      // 作者信息
      authorInfo: {},
      // 热门文章
      hotArticles: [],
  ...
  async asyncData ( {route, req} ) {
    ...
    if(req.session.KGUser) {
      userId = req.session.KGUser.userId
      token = req.session.KGUser.token
    }
    ...
    try {
      ...
      asyncData.authorInfo = Object.assign(asyncData.authorInfo, data3.data.responseBody.data || {}) // 处理咨询详情页右侧作者信息数据
      ...
      // console.log('data5.data.responseBody', data5.data.responseBody)
      // console.log('asyncData.adviertisement', asyncData.adviertisement)
  ...
  methods: {
    ...
    getLinkUrl(position){
      ...
      if(info.imageType == 2) { // 广告
        return this.handleImgAddrHandle(info.adverLink)
      }
