 ...
 * @Last Modified by: cb
 * @Last Modified time: 2018-06-22 11:26:06
...
<div class="authorInfo">
    ...
    <div class="authorDetail">
        ...
        <div class="baseData">
            <div>
                <span  v-if="authorData.artsum>9999">{{Math.floor((authorData.artsum)/1000)/10}}<span class="el_w">w+</span></span>
                <span v-else>{{authorData.artsum}}</span>
                <span>投稿篇数</span>
            </div>
            <div>
                <span  v-if="authorData.comments>9999">{{Math.floor((authorData.comments)/1000)/10}}<span class="el_w">w+</span></span>
                <span  v-else>{{authorData.comments}}</span>
                <span>被评论条数</span>
            </div>
            <div>
                <!-- <span>{{authorData.pbowsenum}}</span> -->
                <span  v-if="authorData.pbowsenum>9999">{{Math.floor((authorData.pbowsenum)/1000)/10}}<span class="el_w">w+</span></span>
                <span  v-else>{{authorData.pbowsenum}}</span>
                <span>总浏览量</span>
            </div>
        </div>
        <img :src="authorData.columnAvatar" :alt="authorData.columnName">
        <!-- <i v-if="item.columnAuthed" class="authedLogo"></i> -->
         <i v-if="authorData.vipTag == 1" class="authedLogo"></i>
    </div>
    <div class="authorArticle">
        ...
        <el-row type="flex" class="row-bg" justify="space-around">
            ...
            <section class="index-info-list">
                <index-list activeName="TA的投稿" :indexTabList="indexTabList" :groupLists="groupLists" @loadingMore="loadingMore"></index-list>
            </section>
            </el-col>
            <el-col :span="6">
...
export default {
  ...
  created () {
    ...
    if(this.authorData.columnAvatar == null || this.authorData.columnAvatar.length === 0 ) {
        this.authorData.columnAvatar = 'https://pro-kg-oss.oss-cn-beijing.aliyuncs.com/author/author_default_avatar.png'
    }
    else {
        this.authorData.columnAvatar  = OSS_ADDRESS + this.authorData.columnAvatar
    }
  },
  mounted(){
  },
  methods: {
    ...
    loadingMore (type) {
      ...
      switch (type) {
        case 'TA的投稿':
          params = handleRequestParams({"createUser":that.userId, "publishStatus":1,"currentPage": this.newsPageNum + 1})
